let goalies_log_table = $("#goalies_log_table").data;
$("#goalies_log_table").data['pageSizesArr'] = [];
$("#goalies_log_table").data['pageNumbersArr'] = [];
goalies_log_table['statType'] = 'skt_log';
goalies_log_table['ascOrderCols'] = [0, 1, 3];
goalies_log_table['headersCount'] = $("#goalies_log_table > thead > tr:first > th").length;
...
$("#goalies_log_table")
  .tablesorter({
    headers: headers(goalies_log_table['headersCount'], goalies_log_table['ascOrderCols'], [], []),
...
.tablesorterPager({
   ...
   customAjaxUrl: function(table, url) {
     urlParts = url.split('?');
     url = urlParts[0] + '/gls_log' + urlParts[1];
     return adjustUrl(url, $("#goalies_log_table"));
   },
});
